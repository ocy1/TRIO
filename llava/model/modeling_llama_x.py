...
class AdapterHead(nn.Module):
    def __init__(self, in_dim, out_dim, hidden=1024, dropout=0.1):
        super().__init__()
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward_x(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1] != 1:  # prefill stage or training
                    # print('self.layer_list:', self.layer_list) # [8, 16, 24]
                    # print('rank_layer:', rank_layer) # 8, 16, 24
                    # print('position_ids:', position_ids) # 0-362
                    # print('attention_mask:', attention_mask) # None
                    # print('hidden_states:', hidden_states.shape) # [1, 362, 4096]
                    ...
                    (
                        ...
                        sum_visual_attention=sum_visual_attention,
                        indices_attention=indices_attention
                    )
                    # print('rank_layer:', rank_layer) # 8, 16, 24
                    # print('position_ids:', position_ids) # 0-152
                    # print('attention_mask:', attention_mask) # None
                    # print('hidden_states:', hidden_states.shape) # [1, 152, 4096]
                    # print('labels:', labels)  # None
                    ...
                    stage = self.layer_list.index(rank_layer) # determine current stage
                    cur_visual_length = [min(int(self.image_token_list[stage]), cur_image_token) for cur_image_token in self.image_tokens] # [576]
                    next_visual_length = [min(int(self.image_token_list[stage + 1]), cur_image_token) for cur_image_token in self.image_tokens] # [288]
        ...
        hidden_states = self.norm(hidden_states)
        # sum_visual_attention = torch.stack(sum_visual_attention, dim=0)
        # indices_attention = torch.stack(indices_attention, dim=0)
        # print('sum_visual_attention:', sum_visual_attention.shape)
        # print('indices_attention:', indices_attention.shape)
        # torch.save(sum_visual_attention, 'sum_visual_attention.pt')
        # torch.save(indices_attention, 'indices_attention.pt')
